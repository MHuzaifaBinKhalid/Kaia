...
# ------------------------------
# Find all JSON files in Extracted_Data
# ------------------------------

data_folder = "ExtractedData"
json_files = [os.path.join(data_folder, f) for f in os.listdir(data_folder) if f.endswith("_6months_posts.json")]
if not json_files:
    raise FileNotFoundError(f"No scraper JSON files found in '{data_folder}' folder!")

print(f"🔹 Found {len(json_files)} JSON files in '{data_folder}': {[os.path.basename(f) for f in json_files]}\n")
...
for json_file in json_files:
    ...
    print(f"📁 Processing {len(posts)} posts from {os.path.basename(json_file)}")
    print("-" * 60)
    ...
    for post in posts:
        ...
        metadata = {
            ...
            "comments_array": post.get("latestComments", [])  # <-- include full comments array
        }
        ...
        if post_type == "Sidecar":
            carousel_media = post.get("carouselMedia", []) or post.get("childPosts", [])
        ...
        # ------------------
            ...
            img_url = post.get("displayUrl") or (post.get("images") or [None])[0]
            if img_url:
